...
@Component({
  selector: 'app-miscomunicados',
  templateUrl: './miscomunicados.component.html',
  styleUrls: ['./miscomunicados.component.css']
})
export class MiscomunicadosComponent implements OnInit, AfterViewInit {
  ...
  borrarComunicado(comunicado: ComunicadoModel, i: number) {
    Swal.fire({
      ...
      if (resp.value) {
        // this.Comunicado.splice(i, 1);
        this.AuthS.DeleteComun(comunicado.ids).subscribe(resp => {
          Swal.fire({
            title: 'Eliminado',
        ...
        // console.log(this.dataSource.data.indexOf(comunicado));
  ...
  borrarPorTokenVencido(err: any, usuario: any): any{
    ...
    if (tokenVencido === "Auth token is expired") {
      ...
      this.AuthS.refrescarToken(refresh).subscribe(resp => {
        ...
        this.AuthS.DeleteComun(usuario).subscribe( resp => {
          // console.log(resp);
          Swal.fire({
            title: 'Eliminado',
